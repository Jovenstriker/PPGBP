...
class SerialPlotter:
    def __init__(self, port, baudrate=115200, max_len = max_length, plot_interval=1, csv_filename=None):
        ...
        self.adc1PlotAvg=deque(maxlen=self.max_len)
        self.adc2PlotAvg=deque(maxlen=self.max_len)
    ...
    def _read_serial(self):
        ...
        while not self._stop_event.is_set():
            ...
            if len(self.adc1)<10:
                self.adc1PlotAvg.append(0)
                self.adc2PlotAvg.append(0)
            else:
                self.adc1PlotAvg.append(np.average(list(self.adc1)[-10:]))
                self.adc2PlotAvg.append(np.average(list(self.adc2)[-10:]))
    ...
    def _update_plots(self, frame):
        ...
        self._line1.set_ydata(self.adc1PlotAvg)
        self._line2.set_xdata(self.timestamps)
        self._line2.set_ydata(self.adc2PlotAvg)
        self._ax1.relim()
...
if __name__ == '__main__':
    ...
    serial_plotter = SerialPlotter(serial_port, max_length, csv_filename='/Users/henrywang/Documents/Python/PrintedResults/'+filename+'.txt')
    
    serial_plotter.start()
    print(str(serial_plotter))
    plt.show(block=True)

    # input('Press Enter to stop...\n')
    serial_plotter.stop()
#COM: /dev/tty.usbmodem1101 wanrong